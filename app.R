...
# (Origin_DB, file='Origin_DB.xlsx', row.names = FALSE)
# check
Origin_DB<-Origin_DB[!is.na(Origin_DB$날짜),]
...
#today_date <- Sys.Date()
today_date  <- as.Date(tail(Origin_DB$날짜,1))
today_date
...
server <- function(input, output, session) {
  ...
  observeEvent(input$input_var_name, {
    ...
    print(input$input_var_name[[1]])
    new_year <- format(as.Date(input$input_var_name[[1]]), "%Y")
